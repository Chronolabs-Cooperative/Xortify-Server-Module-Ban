 ...
 *
 */
class banFormSelectCountry extends XoopsFormElement
{
    /**
     * Options
     *
    ...
    function __construct($caption, $name, $value = null, $size = 1, $multiple = false)
    {
        ...
        if (empty($countries)||count($countries)==0) {
        	if (!$countries = XoopsCache::read('api_countries_list')) {
        		$countries = json_decode(ban_get_curl('http://places.xortify.com/v1/list/list/json.api'), true);
        		XoopsCache::write('api_countries_list', $countries, 3600*24*7*4*3);
        	}
        }
        $this->addOption('', 'No country specified');
        foreach($countries['countries'] as $key => $data)
        {
        	$this->addOption($data['key'], $data['Country']. ' ('.$data['ISO3'].')');
